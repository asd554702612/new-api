  ...
  const data = order
    ? [
        { key: t('订单号'), value: <Text copyable>{order.trade_no}</Text> },
        {
          key: t('订单类型'),
          value: t(
            order.order_type === 'subscription' ? '订阅订单' : '充值订单',
          ),
        },
        { key: t('用户ID'), value: order.user_id },
        {
          key: t('支付方式'),
          value: order.payment_method || order.payment_provider || '-',
        },
        {
          key: t('状态'),
          value: <PaymentOrderStatusTag status={order.status} t={t} />,
        },
        {
          key: t('支付金额'),
          value: formatPaymentOrderMoney(order.pay_amount, order),
        },
            ...
            columns={[
              {
                title: t('金额'),
                dataIndex: 'amount',
                render: (value) => formatPaymentOrderMoney(value, order),
              },
