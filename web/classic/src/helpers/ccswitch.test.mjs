...
import { buildCCSwitchURL } from './ccswitch.js';
...
describe('buildCCSwitchURL', () => {
  test('adds token usage query configuration to imported provider', () => {
    ...
    assert.match(script, /remaining:\s*Number\(data\.total_available\s*\?\?\s*0\)\s*\/\s*quotaPerUnit/);
    assert.match(script, /used:\s*Number\(data\.total_used\s*\?\?\s*0\)\s*\/\s*quotaPerUnit/);
    assert.match(script, /total:\s*Number\(data\.total_granted\s*\?\?\s*0\)\s*\/\s*quotaPerUnit/);
